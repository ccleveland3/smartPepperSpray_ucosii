...
void WaveRecorderUpdate(void)
{
  ...
  //FA_CREATE_ALWAYS should cause the same thing.  It creates a new file and if
  //the file is exsisting then it will be truncated and overwritten.

  /* Open the file to write on it */
  if ((SD_Detect() != SD_PRESENT) || (f_open(&file, REC_WAVE_NAME, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
  {
    /* Set ON Red LED */
    while(1)
    {
  ...
  while(SD_Detect() == SD_PRESENT)
  {
    ...
    if (Time_Rec_Base <= TIME_REC)
    {
      ...
      while((Data_Status == 0)&& (SD_Detect() == SD_PRESENT));
      Data_Status =0;
  ...
  /* Seek to the beginning of the file by closing and then reopening the same
  * file.
  */
  f_close(&file);
  f_open(&file, REC_WAVE_NAME, FA_OPEN_EXISTING | FA_WRITE);

  RecBufHeader[4] = (uint8_t)(  WaveCounter + 36) ;
  RecBufHeader[5] = (uint8_t)(((WaveCounter + 36)  >> 8) & 0xFF);
  RecBufHeader[6] = (uint8_t)(((WaveCounter + 36)  >> 16) & 0xFF);
  RecBufHeader[7] = (uint8_t)(((WaveCounter + 36)  >> 24) & 0xFF);
